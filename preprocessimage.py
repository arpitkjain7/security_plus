...
class PagePreprocess(object):
    def __init__(self, im, image_path: str):
        self.err = False
        self.orig_im = im
    ...
    def deskew(self):
        try:
            self.image, self.theta_est = process_skewed_crop(
...
def angle_calculation(gray):
    ...
    if len(np.where(slopes == 0.0)[0]) >= 2:
        # #logging.error(f"Error: {e}", exc_info=True)don't rotate")
        return None
    else:
        # #logging.error(f"Error: {e}", exc_info=True)rotate")
...
def estimate_skew(image, image_path: str = None):
    try:
        # logging.debug("Estimating skew...")
        if image_path is not None:
            osd = image_to_osd(image_path)
            angle = float(re.search("(?<=Rotate: )\d+", osd).group(0))
            if angle == 0:
                # fin_image = rotate(image_gray, angle)
                edges = auto_canny(image)
                # print(edges.shape)
                # #logging.error(f"Error: {e}", exc_info=True)edges found: ", edges)
                lines = cv2.HoughLines(edges, 1, np.pi / 270, 400)
                # #logging.error(f"Error: {e}", exc_info=True)lines found: ", lines)
        ...
        if type(e) == NotImplementedError:
            print(f"Error in estimate_skew: {e}")
            # logging.error("Send image path to this function, OCR server needs it!")
        # logging.error(f"Error: {e}", exc_info=True)
        return None


def process_skewed_crop(image, image_path: str):
    try:
        theta = estimate_skew(image, image_path)
        ...
        print(f"Error in process_skewed_crop: {e}")
        # logging.error(f"Error: {e}", exc_info=True)


def preprocess_image(file_path: str):
    try:
        # logging.debug(f"preprocessing image {file_path}")
        gray_page = cv2.imread(file_path, 0)
        print(gray_page)
        process_page = PagePreprocess(im=gray_page, image_path=file_path)
        # _ = process_page.crop()
        print(process_page)
        deskewed_page = process_page.deskew()
        print(deskewed_page)
        # logging.debug(f"Writing to disk: {file_path}")
...
def preprocess_image_file(img):
    try:
        # logging.debug(f"preprocessing image file... {type(img)}")
        cvimg = cv2.cvtColor(
            np.array(img), cv2.COLOR_RGB2BGR
        )  # converting PIL to Cv2 image
        gray_page = cv2.cvtColor(cvimg, cv2.COLOR_BGR2GRAY)  # cv2 grey scale
